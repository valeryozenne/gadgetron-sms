...
#include "BucketToBufferSMSGadget.h"
#include "mri_core_data.h"
#include "hoNDArray_elemwise.h"
#include "hoNDArray_reductions.h"
namespace Gadgetron {

    BucketToBufferSMSGadget::BucketToBufferSMSGadget()
    {
    }
        ...
        ::process(GadgetContainerMessage<IsmrmrdAcquisitionBucket>* m1)
    {
        ...
        //GDEBUG("BucketToBufferSMSGadget::process\n");
        ...
        //Iterate over the reference data of the bucket
           ...
           for (std::vector<IsmrmrdAcquisitionData>::iterator it = m1->getObjectPtr()->sb_.begin();
                it != m1->getObjectPtr()->sb_.end(); ++it)
           {
               //Get a reference to the header for this acquisition
               ISMRMRD::AcquisitionHeader & acqhdr = *it->head_->getObjectPtr();
               ...
               //and the corresponding data buffer for the reference data
               if (!rbit.sb_)
                   rbit.sb_ = IsmrmrdDataBuffered();
               IsmrmrdDataBuffered & dataBuffer = *rbit.sb_;
               //this encoding space's xml header info
               ISMRMRD::Encoding & encoding = hdr_.encoding[espace];
               //this bucket's reference stats
               IsmrmrdAcquisitionBucketStats & stats = m1->getObjectPtr()->sbstats_[espace];
    ...
    int BucketToBufferSMSGadget::close(unsigned long flags)
    {

        int ret = Gadget::close(flags);
        GDEBUG("BucketToBufferSMSGadget::close\n");

        return ret;
    }

    size_t BucketToBufferSMSGadget::getSlice(ISMRMRD::ISMRMRD_EncodingCounters idx)
    {
        size_t index;
    ...
    size_t BucketToBufferSMSGadget::getN(ISMRMRD::ISMRMRD_EncodingCounters idx)
    {
        size_t index;
    ...
    size_t BucketToBufferSMSGadget::getS(ISMRMRD::ISMRMRD_EncodingCounters idx)
    {
        size_t index;
    ...
    size_t BucketToBufferSMSGadget::getKey(ISMRMRD::ISMRMRD_EncodingCounters idx)
    {
    ...
    IsmrmrdReconBit & BucketToBufferSMSGadget::getRBit(std::map<size_t, GadgetContainerMessage<IsmrmrdReconData>* > & recon_data_buffers, size_t key, uint16_t espace)
    {
    ...
    void BucketToBufferSMSGadget::allocateDataArrays(IsmrmrdDataBuffered & dataBuffer, ISMRMRD::AcquisitionHeader & acqhdr, ISMRMRD::Encoding encoding, IsmrmrdAcquisitionBucketStats & stats, bool forref)
    {
    ...
    void BucketToBufferSMSGadget::fillSamplingDescription(SamplingDescription & sampling, ISMRMRD::Encoding & encoding, IsmrmrdAcquisitionBucketStats & stats, ISMRMRD::AcquisitionHeader& acqhdr, bool forref)
    {
    ...
    void BucketToBufferSMSGadget::stuff(std::vector<IsmrmrdAcquisitionData>::iterator it, IsmrmrdDataBuffered & dataBuffer, ISMRMRD::Encoding encoding, IsmrmrdAcquisitionBucketStats & stats, bool forref)
    {
    ...
    GADGET_FACTORY_DECLARE(BucketToBufferSMSGadget)

...
#include "GenericReconSMSPostv0Gadget.h"
#include <iomanip>
...
namespace Gadgetron {
    ...
    GenericReconSMSPostv0Gadget::GenericReconSMSPostv0Gadget() : BaseClass()
    {
    }

    GenericReconSMSPostv0Gadget::~GenericReconSMSPostv0Gadget()
    {
    }

    int GenericReconSMSPostv0Gadget::process_config(ACE_Message_Block* mb)
    {
    ...
    int GenericReconSMSPostv0Gadget::process(Gadgetron::GadgetContainerMessage< IsmrmrdReconData >* m1)
    {
        if (perform_timing.value()) { gt_timer_.start("GenericReconSMSPostv0Gadget::process"); }
        ...
        for (e = 0; e < recon_bit_->rbit_.size(); e++)
        {
            ...
            if (recon_bit_->rbit_[e].ref_)
            {
               // std::cout << " je suis la structure qui contient les données acs" << std::endl;
                ...
                GDEBUG_STREAM("GenericReconSMSPostv0Gadget - incoming data array ref : [RO E1 E2 CHA N S SLC] - [" << RO << " " << E1 << " " << E2 << " " << CHA << " " << N << " " << S << " " << SLC << "]");
            }
            ...
            if (recon_bit_->rbit_[e].sb_)
            {
               // std::cout << " je suis la structure qui contient les données single band" << std::endl;
                ...
                GDEBUG_STREAM("GenericReconSMSPostv0Gadget - incoming data array sb : [RO E1 E2 CHA MB STK N S] - [" << RO << " " << E1 << " " << E2 << " " << CHA << " " << MB << " " << STK << " " << N << " " << S <<"]");
    ...
    void GenericReconSMSPostv0Gadget::undo_stacks_ordering_to_match_gt_organisation(hoNDArray< std::complex<float> >& data, hoNDArray< std::complex<float> > &output)
        {
            size_t RO=data.get_size(0);
            size_t E1=data.get_size(1);
            size_t E2=data.get_size(2);
    ...
    GADGET_FACTORY_DECLARE(GenericReconSMSPostv0Gadget)
}

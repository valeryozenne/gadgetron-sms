...
#include "GenericReconSMSPrepGadget_1of2.h"
#include <iomanip>
...
GenericReconSMSPrepGadget_1of2::GenericReconSMSPrepGadget_1of2() : BaseClass()
{
}

GenericReconSMSPrepGadget_1of2::~GenericReconSMSPrepGadget_1of2()
{
}
    ...
    for (e = 0; e < recon_bit_->rbit_.size(); e++)
    {
        ...
        if (rbit.data_.data_.get_number_of_elements() > 0)
        {
            ...
            else
            {
                ...
                GDEBUG_STREAM("GenericReconSMSPrepGadget_1of2 - incoming data array mb : [RO E1 E2 CHA N S SLC  ] - [" << RO << " " << E1 << " " << E2 << " " << CHA << " " << N << " " << S << " " << SLC << "]");
...
void GenericReconSMSPrepGadget_1of2::pre_process_ref_data(hoNDArray< std::complex<float> >& ref, hoNDArray< std::complex<float> >& ref_8D, size_t e)
{
...
void GenericReconSMSPrepGadget_1of2::pre_process_sb_data(hoNDArray< std::complex<float> >& sb, hoNDArray< std::complex<float> >& sb_8D, hoNDArray< ISMRMRD::AcquisitionHeader > & h_sb, size_t e)
{
...
void GenericReconSMSPrepGadget_1of2::pre_process_mb_data(hoNDArray< std::complex<float> >& mb, hoNDArray< std::complex<float> >& mb_8D,hoNDArray< ISMRMRD::AcquisitionHeader > & h_mb, size_t e)
{
    // three steps are necessary
    // 1) to reorganize the slices in the stack of slices according the to the slice acceleration
...
void GenericReconSMSPrepGadget_1of2::reorganize_sb_data_to_8D(hoNDArray< std::complex<float> >& sb, hoNDArray< std::complex<float> >& sb_8D, size_t e)
{
    std::stringstream os;
    os << "_encoding_" << e;
    ...
    {
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::create_stacks_of_slices_directly_open"); }
        create_stacks_of_slices_directly_sb_open(sb, sb_8D, indice_sb, MapSliceSMS);
        if (perform_timing.value()) { gt_timer_local_.stop(); }
    }
    else
    {
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::create_stacks_of_slices_directly"); }
...
void GenericReconSMSPrepGadget_1of2::reorganize_mb_data_to_8D(hoNDArray< std::complex<float> >& mb, hoNDArray< std::complex<float> >& mb_8D, size_t e)
{
    ...
    if (use_omp.value()==true)
    {
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::create_stacks_of_slices_directly_mb_open"); }
        create_stacks_of_slices_directly_mb_open(mb, mb_8D,  indice_mb, indice_slice_mb);
        if (perform_timing.value()) { gt_timer_local_.stop(); }
    }
    else
    {
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::create_stacks_of_slices_directly_mb"); }
...
void GenericReconSMSPrepGadget_1of2::apply_blip_caipi_shift_sb(hoNDArray< std::complex<float> >& sb_8D, hoNDArray< ISMRMRD::AcquisitionHeader > & headers_sb, size_t e)
{
    std::stringstream os;
    os << "_encoding_" << e;
    std::string suffix = os.str();
    ...
    if (is_wip_sequence==1)
    {
        ...
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::apply_relative_phase_shift"); }
        ...
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::get_header_and_position_and_gap"); }
        get_header_and_position_and_gap(sb_8D, headers_sb);
        if (perform_timing.value()) { gt_timer_local_.stop(); }

        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::apply_absolute_phase_shift"); }
...
void GenericReconSMSPrepGadget_1of2::apply_averaged_epi_ghost_correction_sb(hoNDArray< std::complex<float> >& sb_8D, hoNDArray< ISMRMRD::AcquisitionHeader > & headers_sb, size_t e)
{
    std::stringstream os;
    os << "_encoding_" << e;
    std::string suffix = os.str();
    ...
    //prepare epi data
    if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::prepare_epi_data"); }
    ...
    if (use_gpu.value())
    {
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::apply_ghost_correction_with_STK6 gpu time Prep SB "); }
    ...
    else
    {
        if (use_omp.value())
        {
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::apply_ghost_correction_with_STK6 openmp time Prep SB "); }
        apply_ghost_correction_with_STK6_open(sb_8D, headers_sb ,  acceFactorSMSE1_[e], false , false, false, "Prep SB");
        if (perform_timing.value()) { gt_timer_local_.stop(); }
        }
        else
        {
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::apply_ghost_correction_with_STK6 cpu time Prep SB "); }
...
void GenericReconSMSPrepGadget_1of2::apply_averaged_epi_ghost_correction_mb(hoNDArray< std::complex<float> >& mb_8D, hoNDArray< ISMRMRD::AcquisitionHeader > & headers_mb, size_t e)
{
    std::stringstream os;
    os << "_encoding_" << e;
    std::string suffix = os.str();
    ...
    {
        if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::apply_ghost_correction_with_STK6 gpu time Prep MB "); }
    ...
    else
    {
        ...
        if (use_omp.value()==true)
        {
            if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::apply_ghost_correction_with_STK6 openmp time Prep MB "); }
            apply_ghost_correction_with_STK6_open(mb_8D, headers_mb ,  acceFactorSMSE1_[e], false , false , false, " Prep MB ");
            if (perform_timing.value()) { gt_timer_local_.stop(); }
        }
        else
        {
            if (perform_timing.value()) { gt_timer_local_.start("GenericReconSMSPrepGadget_1of2::apply_ghost_correction_with_STK6 cpu time Prep MB "); }
...
GADGET_FACTORY_DECLARE(GenericReconSMSPrepGadget_1of2)
}

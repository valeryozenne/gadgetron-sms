...
SortSliceGadget::SortSliceGadget() : BaseClass()
{
}

SortSliceGadget::~SortSliceGadget()
{
}

int SortSliceGadget::process_config(ACE_Message_Block* mb)
{
...
int SortSliceGadget::process(Gadgetron::GadgetContainerMessage< IsmrmrdImageArray >* m1)
{
    if (perform_timing.value()) { gt_timer_local_.start("SortSliceGadget::process"); }

    GDEBUG_CONDITION_STREAM(verbose.value(), "SortSliceGadget::process(...) starts ... ");
    ...
    for  (size_t slc = 0; slc <  hSLC; slc++) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                slice_dir_all(j,slc)=curr_image_header.slice_dir[j];
            }
    ...
    for  (size_t slc = 0; slc <  hSLC; slc++) {
        ...
        size_t new_index=order_of_acquisition_sb[slc];


        GDEBUG_STREAM( "slc "<<slc  << " repetition :  "  << repetition  <<  " image_index :  "  << image_index   <<  " image_series_index :  "  << image_series_index  );
        GDEBUG_STREAM( "slc "<<slc  << " shift_from_isocenter  " <<shift_from_isocenter(0) <<"  "<< shift_from_isocenter(1) <<"  "<<shift_from_isocenter(2) <<"   z_offset(slc) "<<  z_offset(slc)<< " order_of_acquisition_sb  "<<new_index << " indice _sb "<<indice_sb[slc]);
     ...
     for  (size_t slc = 0; slc <  hSLC; slc++) {
        ...
        // par ex SRC: slc = 7 -> DEST index[slc] =0
        std::complex<float> *pSrc = &(temporaire(0, 0, 0, 0, 0, 0, slc ));
        std::complex<float> *pDst = &(data->data_(0, 0, 0, 0, 0, 0, new_index));
        memcpy(pDst, pSrc, sizeof(std::complex<float>) * RO * E1 * E2 * CHA * N * S );

        for (int j = 0; j < 3; j++) {

            data->headers_(0, 0,  new_index).position[j]=shift_from_isocenter_all(j,slc);
            data->headers_(0, 0,  new_index).slice_dir[j]=slice_dir_all(j,slc);
        }
    ...
    if (verbose.value())
    {
        GDEBUG_STREAM("----> SortSliceGadget::process(...) has been called " << process_called_times_ << " times ...");
        std::stringstream os;
        data->data_.print(os);
    ...
    {
        GERROR("SortSliceGadget::process, passing map on to next gadget");
        return GADGET_FAIL;
    }
...
std::vector<unsigned int> SortSliceGadget::sort_index(std::vector<unsigned int> array)
{
...
std::vector<unsigned int> SortSliceGadget::map_interleaved_acquisitions(int number_of_slices, bool no_reordering )
{
...
GADGET_FACTORY_DECLARE(SortSliceGadget)

...
#include "EPICorrMultiBandCMRRGadget.h"
...
EPICorrMultiBandCMRRGadget::EPICorrMultiBandCMRRGadget() {}
EPICorrMultiBandCMRRGadget::~EPICorrMultiBandCMRRGadget() {}

int EPICorrMultiBandCMRRGadget::process_config(ACE_Message_Block* mb)
{
    ...
    Gadgetron::DeleteTemporaryFiles( str_home, "Tempo/", "corrpos_*" );
    Gadgetron::DeleteTemporaryFiles( str_home, "Tempo/", "corrneg_*" );
    ...
    //std::cout <<  " lNumberOfStacks_ " <<  lNumberOfStacks_  <<  " lNumberOfSlices_ " <<  lNumberOfSlices_   << std::endl;
    ...
    for ( int s = 0; s < lNumberOfSlices_; s++)
    {

        GDEBUG("gt numero %d spatial numero %d stack (from gt numero) %d  stack (from spatial numero) %d  blipped %d \n", s , get_spatial_slice_number(s), get_stack_number_from_gt_numerotation(s),  get_stack_number_from_spatial_numerotation(s),  get_blipped_factor(s) );
    }
    ...
    ///
    ///
    GDEBUG_STREAM("EPICorrMultiBandCMRRGadget configured");
    return 0;
}

int EPICorrMultiBandCMRRGadget::process(
        ...
        GadgetContainerMessage< hoNDArray< std::complex<float> > >* m2)
{
    ...
    if (hdr.encoding_space_ref > 0) {
        ...
        if (this->next()->putq(m1) == -1) {
            m1->release();
            GERROR("EPICorrMultiBandCMRRGadget::process, passing data on to next gadget");
            return -1;
        }
        return 0;
    }
    ...
    if (hdr.isFlagSet(ISMRMRD::ISMRMRD_ACQ_IS_PHASECORR_DATA)) {
        ...
        if (navNumber_ == (numNavigators_-1)) {
            ...
            {                
                ...
                corrpos_no_exp_save_.col(slice) = arma::cx_fvec(arma::zeros<arma::fvec>(Nx_), -0.5*tvec);
                corrneg_no_exp_save_.col(slice) = arma::cx_fvec(arma::zeros<arma::fvec>(Nx_), +0.5*tvec);
               ...
               // std::cout<< " slice  "<< slice  <<  "  order_of_acquisition_sb(slice)  " << order_of_acquisition_sb(slice) << "  " << get_blipped_factor(order_of_acquisition_sb(slice)) << "  Blipped_CAIPI_-1 :  "  << (Blipped_CAIPI_-1)   << std::endl;

                // on peut le faire uniquement si on est au dernier stack
                if (get_blipped_factor(order_of_acquisition_sb(slice))==(MB_factor_-1))
                {
                    corrpos_mean_.col(get_stack_number_from_gt_numerotation(slice))=arma::exp(corrpos_no_exp_mean_.col(get_stack_number_from_gt_numerotation(slice)));
                    corrneg_mean_.col(get_stack_number_from_gt_numerotation(slice))=arma::exp(corrneg_no_exp_mean_.col(get_stack_number_from_gt_numerotation(slice)));
                }
            }
        ...
        {
            // on imprime au fur et à mesure
            // si on est a la derniere coupe de la slice de calibration
            // on somme les corrections EPI et on imprime

            // on imprime le tout par slices
            ...
            Gadgetron::SaveVectorOntheDisk(corrneg_, str_home, "Tempo/", "corrneg_",   str_s,  ".bin");
            Gadgetron::SaveVectorOntheDisk(corrpos_, str_home, "Tempo/", "corrpos_",   str_s,  ".bin");

            Gadgetron::SaveVectorOntheDisk(corrneg_no_exp_save_.col(slice), str_home, "Tempo/", "corrneg_no_exp_",   str_s,  ".bin");
            Gadgetron::SaveVectorOntheDisk(corrpos_no_exp_save_.col(slice), str_home, "Tempo/", "corrpos_no_exp_",   str_s,  ".bin");

            std::ostringstream indice_stack;
            indice_stack << get_stack_number_from_gt_numerotation(slice);
            str_a = indice_stack.str();

            //std::cout<< " slice  "<< slice  <<  "  order_of_acquisition_sb(slice)  " << order_of_acquisition_sb(slice) << " get blipped  " << get_blipped_factor(order_of_acquisition_sb(slice)) << "  Blipped_CAIPI_-1 :  "  << (Blipped_CAIPI_-1)   << std::endl;


            // attention avant c'est Blip_Caipi
            if (get_blipped_factor(order_of_acquisition_sb(slice))==(MB_factor_-1))
            {
                GDEBUG(" Get_blipped_factor : %d  , saving mean of EPI phase correction for stack n° %d \n" ,  get_blipped_factor(order_of_acquisition_sb(slice)), get_stack_number_from_gt_numerotation(slice));
                ...
                Gadgetron::SaveVectorOntheDisk(corrneg_mean_.col(get_stack_number_from_gt_numerotation(slice)), str_home, "Tempo/", "corrneg_mean_",   str_a,  ".bin");
                Gadgetron::SaveVectorOntheDisk(corrpos_mean_.col(get_stack_number_from_gt_numerotation(slice)), str_home, "Tempo/", "corrpos_mean_",   str_a,  ".bin");
            }
        }
...
*/
            ...
            else
            {
                ...
                {
                    deja_vu_stacks(get_stack_number_from_gt_numerotation(slice))++;

                    GDEBUG("Buffering slice :  %d , spatial slice position : %d ,   stack number : %d ,   indice back : %d   \n", slice ,  get_spatial_slice_number(slice), get_stack_number_from_gt_numerotation(slice)  , indice_sb(get_spatial_slice_number(slice)));
                    //std::cout << " slice  "<< slice  << " spatial " << get_spatial_slice_number(slice) << "  stack number " <<  get_stack_number_from_gt_numerotation(slice) << std::endl;

                }

                slice_calibration.slice(slice).col(e1)=vectorise(adata);
                matrix_encoding(e1,slice)++;

                if (hdr.isFlagSet(ISMRMRD::ISMRMRD_ACQ_IS_REVERSE))
                {
                    reverse_encoding(e1,slice)++;
                }

                // si premier slice
                if (slice==indice_sb(0))
                {                    
                detect_flag(m1);
                }

                //if (is_last_scan_in_slice && (deja_vu_stacks(get_stack_number_from_gt_numerotation(slice))==MB_factor_))
                //{
                //int stacks_number= get_stack_number_from_gt_numerotation(slice);

                //std::cout << " !! slice  "<< slice  << " spatial " << get_spatial_slice_number(slice) << "  stack number " <<  stacks_number << std::endl;

                //std::cout << MapSliceSMS.col(stacks_number)<<  std::endl;
                //std::cout << MapSliceSMS.col(stacks_number).rows(0, MB_factor_-2)<<  std::endl;

                //arma::ivec slices_to_send_back=MapSliceSMS.col(stacks_number).rows(0, MB_factor_-1);

                //transfer_kspace_to_the_next_gadget(m1, m2 , slices_to_send_back);

                //}
                //std::cout << "slice " << slice << "   " << indice_sb(lNumberOfSlices_-1) <<"   "<< get_spatial_slice_number(slice)  << std::endl;

                if (is_last_scan_in_slice && (slice==indice_sb(lNumberOfSlices_-1)))
                {
                    GDEBUG("Condition indice slice %d == %d , spatial slice position %d ,  starting sending back slice   \n", slice ,  indice_sb(lNumberOfSlices_-1), get_spatial_slice_number(slice));

                    for (int ss=0; ss < lNumberOfSlices_; ss++) {

                        transfer_kspace_to_the_next_gadget_2(m1, m2 , ss);
                    }
                }
            }
            ...
            //TODO on aurait pu prendre une autre condition
            /*if (deja_vu_stacks(get_stack_number_from_gt_numerotation(slice))==MB_factor_)
            {
                // on est sur la dernier slice du stack la correction EPI moyenne est deja calcule on peut donc l'appliquer au donne single band
                ...
                if (hdr.isFlagSet(ISMRMRD::ISMRMRD_ACQ_IS_REVERSE)) {
                    ...
                    for (int p=0; p<adata.n_cols; p++) {
                        // adata.col(p) %= (arma::pow(corrB0_,epiEchoNumber_+RefNav_to_Echo0_time_ES_) % corrneg_);
                        // cout << "sb corrneg_"<< size(corrneg_)<<"  " <<  abs(corrneg_).max() << std::endl;
                        //adata.col(p) %=  corrneg_mean_.col(get_stack_number_from_gt_numerotation(slice));
                    }
                    // Now that we have corrected we set the readout direction to positive
                }
                else {
                    ...
                    for (int p=0; p<adata.n_cols; p++) {
                        // adata.col(p) %= (arma::pow(corrB0_,epiEchoNumber_+RefNav_to_Echo0_time_ES_) % corrpos_);
                        // cout << "sb corrpos_"<< size(corrpos_)<<"  " <<  abs(corrpos_).max() << std::endl;
                        // adata.col(p) %=  corrpos_mean_.col(get_stack_number_from_gt_numerotation(slice));
                    }
                }
                ...
                if (is_last_scan_in_slice )
                {
                    ...
                    arma::ivec slices_to_send_back=MapSliceSMS.col(stacks_number).rows(0, MB_factor_-2);
            ...
            {
                // on bufferize les données
                if (is_first_scan_in_slice)
                {
                    std::cout << "buffering slice  "<< slice  << " spatial " << get_spatial_slice_number(slice) << "  stack number " <<  get_stack_number_from_gt_numerotation(slice) << " indice back   :" <<  indice_sb(get_spatial_slice_number(slice)) << std::endl;
                }

                slice_calibration.slice(get_spatial_slice_number(slice)).col(e1)=vectorise(adata);
                matrix_encoding(e1,get_spatial_slice_number(slice))++;
            }*/
        ...
        {
            // deux cas SB ou MB
            // dans les deux cas on applique la correction moyennee par stack
            ...
            if (hdr.isFlagSet(ISMRMRD::ISMRMRD_ACQ_IS_REVERSE)) {
                ...
                for (int p=0; p<adata.n_cols; p++) {
                    // adata.col(p) %= (arma::pow(corrB0_,epiEchoNumber_+RefNav_to_Echo0_time_ES_) % corrneg_);
                    // cout << " "  << get_stack_number_from_gt_numerotation(slice)  <<  " mb corrneg_mean_ " <<  abs(corrneg_mean_.col(get_stack_number_from_gt_numerotation(slice))).max() << std::endl;
                    adata.col(p) %=  corrneg_mean_.col(get_stack_number_from_gt_numerotation(slice));
                }
                // Now that we have corrected we set the readout direction to positive
                //hdr.clearFlag(ISMRMRD::ISMRMRD_ACQ_IS_REVERSE);
                ...
                for (int p=0; p<adata.n_cols; p++) {
                    // adata.col(p) %= (arma::pow(corrB0_,epiEchoNumber_+RefNav_to_Echo0_time_ES_) % corrpos_);
                    // cout << " "  << get_stack_number_from_gt_numerotation(slice)  <<  " mb corrneg_mean_ " <<  abs(corrpos_mean_.col(get_stack_number_from_gt_numerotation(slice))).max() << std::endl;
                    adata.col(p) %=  corrpos_mean_.col(get_stack_number_from_gt_numerotation(slice));
                }
            }
...
void EPICorrMultiBandCMRRGadget::init_arrays_for_nav_parameter_filtering( ISMRMRD::EncodingLimits e_limits )
{
...
float EPICorrMultiBandCMRRGadget::filter_nav_correction_parameter( hoNDArray<float>& nav_corr_param_array,
                                                                   ...
                                                                   bool   filter_in_complex_domain )
{
    ...
    {
        GERROR("cbi_EPICorrMultiBandCMRRGadget::filter_nav_correction_parameter, incorrect number of dimensions of the array.\n");
        return -1;
    }
    ...
    {
        GERROR("cbi_EPICorrMultiBandCMRRGadget::filter_nav_correction_parameter, dimensions of the parameter and weights arrays don't match.\n");
        return -1;
    }
...
void EPICorrMultiBandCMRRGadget::increase_no_repetitions( size_t delta_rep )
{

    GDEBUG_STREAM("cbi_EPICorrMultiBandCMRRGadget WARNING: repetition number larger than what specified in header");
...
int EPICorrMultiBandCMRRGadget::get_stack_number_from_gt_numerotation(int slice)
{
...
int EPICorrMultiBandCMRRGadget::get_stack_number_from_spatial_numerotation(int slice)
{
...
int EPICorrMultiBandCMRRGadget::get_blipped_factor(int numero_de_slice)
{
...
void EPICorrMultiBandCMRRGadget::get_multiband_parameters(ISMRMRD::IsmrmrdHeader h)
{
...
int EPICorrMultiBandCMRRGadget::get_spatial_slice_number(int slice)
{
...
int EPICorrMultiBandCMRRGadget::transfer_kspace_to_the_next_gadget_2(GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *m1, GadgetContainerMessage<hoNDArray<std::complex<float> > > *m2 , unsigned int index)
{

    GDEBUG("Indice: %d , sending back slice : %d   \n",  index ,  indice_sb(index));
    ...
    for (unsigned long i = 0; i < e1_size_for_epi_calculation; i++)
    {
        if(matrix_encoding(i,indice_sb(index))>0)  // ce flag sert a ne pas envoyer les lignes vide dans le cas du grappa
        {
            ...
            *cm1->getObjectPtr() = *m1->getObjectPtr();
            ...
            arma::cx_fmat input= slice_calibration.slice(indice_sb(index)).col(i);
            ...
            if (reverse_encoding(i,indice_sb(index))>0)
            {
                ...
                for (int p=0; p<output.n_cols; p++) {
                    //adata.col(p) %= (arma::pow(corrB0_,epiEchoNumber_+RefNav_to_Echo0_time_ES_) % corrneg_);
                    output.col(p) %=  corrneg_mean_.col(get_stack_number_from_gt_numerotation(indice_sb(index)));
                ...
                for (int p=0; p<output.n_cols; p++) {
                    //adata.col(p) %= (arma::pow(corrB0_,epiEchoNumber_+RefNav_to_Echo0_time_ES_) % corrpos_);
                    output.col(p) %=  corrpos_mean_.col(get_stack_number_from_gt_numerotation(indice_sb(index)));
            ...
            if((indice_sb(index)==indice_sb(lNumberOfSlices_-1)) && (i==flag_encoding_last_scan_in_slice))
            {
                //std::cout << "ISMRMRD::ISMRMRD_ACQ_LAST_IN_REPETITION " << index << " "<< indice_sb(lNumberOfSlices_-1) <<std::endl;
            ...
            cm1->getObjectPtr()->idx.slice = indice_sb(index) ;
...
void EPICorrMultiBandCMRRGadget::detect_flag(GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *m1)
{
...
void EPICorrMultiBandCMRRGadget::find_encoding_dimension_for_epi_calculation(void)
{
    if(acceFactorE1_>1)
    {
...
void EPICorrMultiBandCMRRGadget::deal_with_inline_or_offline_situation(ISMRMRD::IsmrmrdHeader h)
{
...
GADGET_FACTORY_DECLARE(EPICorrMultiBandCMRRGadget)
}

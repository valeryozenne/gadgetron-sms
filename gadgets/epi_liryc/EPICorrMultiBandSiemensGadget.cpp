...
#include "EPICorrMultiBandSiemensGadget.h"
...
EPICorrMultiBandSiemensGadget::EPICorrMultiBandSiemensGadget() {}
EPICorrMultiBandSiemensGadget::~EPICorrMultiBandSiemensGadget() {}

int EPICorrMultiBandSiemensGadget::process_config(ACE_Message_Block* mb)
{
    ...
    ///
    ///
    GDEBUG_STREAM("EPICorrMultiBandSiemensGadget configured");
    return 0;
}

int EPICorrMultiBandSiemensGadget::process(
        ...
        GadgetContainerMessage< hoNDArray< std::complex<float> > >* m2)
{
    ...
    if (hdr.encoding_space_ref > 0) {
        ...
        if (this->next()->putq(m1) == -1) {
            m1->release();
            GERROR("EPICorrMultiBandSiemensGadget::process, passing data on to next gadget");
            return -1;
        }
        return 0;
    }
...
*/
 ...
 //
                ...
                if (is_last_scan_in_slice && (compteur_sb_sum == lNumberOfSlices_) )
                {
                    GDEBUG("STEP 4 : Condition indice compteur_sb_sum %d == %d  lNumberOfSlices_, slice %d , spatial slice position %d    \n", compteur_sb_sum ,  lNumberOfSlices_,  slice, get_spatial_slice_number(slice));
                    GDEBUG("STEP 4 : --> Starting sending back SB slice   \n");

                    if (flag_average_ghost_niquist_correction_are_available==0)
                    {
                        GERROR("EPICorrMultiBandSiemensGadget::process, average_ghost_niquist_correction_are_not_available for SB imges not transfering SB images");
                        return -1;
                    }

                    for (int ss=0; ss < lNumberOfSlices_; ss++) {

                        // on applique ici la correction EPI sur les slices
        ...
        else if (is_acq_multi_band)
        {
            ...
            // on applique la correction moyennee par stack

            if (flag_average_ghost_niquist_correction_are_available==0)
            {
                GERROR("EPICorrMultiBandSiemensGadget::process, average_ghost_niquist_correction_are_not_available for MB iamges ");
                return -1;
            }
...
void EPICorrMultiBandSiemensGadget::init_arrays_for_nav_parameter_filtering( ISMRMRD::EncodingLimits e_limits )
{
...
float EPICorrMultiBandSiemensGadget::filter_nav_correction_parameter( hoNDArray<float>& nav_corr_param_array,
                                                                   ...
                                                                   bool   filter_in_complex_domain )
{
    ...
    {
        GERROR("cbi_EPICorrMultiBandSiemensGadget::filter_nav_correction_parameter, incorrect number of dimensions of the array.\n");
        return -1;
    }
    ...
    {
        GERROR("cbi_EPICorrMultiBandSiemensGadget::filter_nav_correction_parameter, dimensions of the parameter and weights arrays don't match.\n");
        return -1;
    }
...
void EPICorrMultiBandSiemensGadget::increase_no_repetitions( size_t delta_rep )
{

    GDEBUG_STREAM("cbi_EPICorrMultiBandSiemensGadget WARNING: repetition number larger than what specified in header");
...
int EPICorrMultiBandSiemensGadget::get_stack_number_from_gt_numerotation(int slice)
{
...
int EPICorrMultiBandSiemensGadget::get_stack_number_from_spatial_numerotation(int slice)
{
...
int EPICorrMultiBandSiemensGadget::get_blipped_factor(int numero_de_slice)
{
...
void EPICorrMultiBandSiemensGadget::get_multiband_parameters(ISMRMRD::IsmrmrdHeader h)
{
...
int EPICorrMultiBandSiemensGadget::get_spatial_slice_number(int slice)
{
...
int EPICorrMultiBandSiemensGadget::transfer_kspace_to_the_next_gadget_2(GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *m1, GadgetContainerMessage<hoNDArray<std::complex<float> > > *m2 , unsigned int index)
{
...
void EPICorrMultiBandSiemensGadget::detect_flag(GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *m1)
{
...
void EPICorrMultiBandSiemensGadget::find_encoding_dimension_for_epi_calculation(void)
{
    if(acceFactorE1_>1)
    {
...
void EPICorrMultiBandSiemensGadget::deal_with_inline_or_offline_situation(ISMRMRD::IsmrmrdHeader h)
{
...
void EPICorrMultiBandSiemensGadget::fonction_qui_sauvegarde_en_memoire_et_somme_les_corrections(int slice, int Nx_local, arma::fvec tvec_local )
{
...
void EPICorrMultiBandSiemensGadget::fonction_qui_sauvegarde_sur_le_disk_les_corrections_par_coupes(int slice )
{
...
void EPICorrMultiBandSiemensGadget::fonction_qui_sauvegarde_sur_le_disk_les_corrections_par_stacks(int slice )
{
...
GADGET_FACTORY_DECLARE(EPICorrMultiBandSiemensGadget)
}

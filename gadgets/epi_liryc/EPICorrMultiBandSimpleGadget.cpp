...
#include "EPICorrMultiBandSimpleGadget.h"
...
EPICorrMultiBandSimpleGadget::EPICorrMultiBandSimpleGadget() {}
EPICorrMultiBandSimpleGadget::~EPICorrMultiBandSimpleGadget() {}

int EPICorrMultiBandSimpleGadget::process_config(ACE_Message_Block* mb)
{
    ...
    ///
    ///
    GDEBUG_STREAM("EPICorrMultiBandSimpleGadget configured");
    return 0;
}

int EPICorrMultiBandSimpleGadget::process(
        ...
        GadgetContainerMessage< hoNDArray< std::complex<float> > >* m2)
{
    ...
    if (hdr.encoding_space_ref > 0) {
        ...
        if (this->next()->putq(m1) == -1) {
            m1->release();
            GERROR("EPICorrMultiBandSimpleGadget::process, passing data on to next gadget");
            return -1;
        }
        return 0;
    }
    ...
    if (hdr.isFlagSet(ISMRMRD::ISMRMRD_ACQ_IS_PHASECORR_DATA)) {
        ...
        if (navNumber_ == (numNavigators_-1)) {
            ...
            if (is_acq_single_band) //sinon c'est inutile
            {
                ...
                fonction_qui_sauvegarde_en_memoire_et_ne_somme_pas_les_corrections(slice, Nx_, tvec);
...
void EPICorrMultiBandSimpleGadget::init_arrays_for_nav_parameter_filtering( ISMRMRD::EncodingLimits e_limits )
{
...
float EPICorrMultiBandSimpleGadget::filter_nav_correction_parameter( hoNDArray<float>& nav_corr_param_array,
                                                                   ...
                                                                   bool   filter_in_complex_domain )
{
    ...
    {
        GERROR("cbi_EPICorrMultiBandSimpleGadget::filter_nav_correction_parameter, incorrect number of dimensions of the array.\n");
        return -1;
    }
    ...
    {
        GERROR("cbi_EPICorrMultiBandSimpleGadget::filter_nav_correction_parameter, dimensions of the parameter and weights arrays don't match.\n");
        return -1;
    }
...
void EPICorrMultiBandSimpleGadget::increase_no_repetitions( size_t delta_rep )
{

    GDEBUG_STREAM("cbi_EPICorrMultiBandSimpleGadget WARNING: repetition number larger than what specified in header");
...
void EPICorrMultiBandSimpleGadget::fonction_qui_sauvegarde_en_memoire_et_ne_somme_pas_les_corrections(int slice, int Nx_local, arma::fvec tvec_local )
{
...
void EPICorrMultiBandSimpleGadget::fonction_qui_sauvegarde_sur_le_disk_les_corrections_par_coupes(int slice )
{
...
GADGET_FACTORY_DECLARE(EPICorrMultiBandSimpleGadget)
}

#include "EPIReconXStudyGadget.h"
...
EPIReconXStudyGadget::EPIReconXStudyGadget() {}
EPIReconXStudyGadget::~EPIReconXStudyGadget() {}

int EPIReconXStudyGadget::process_config(ACE_Message_Block* mb)
{
    ...
    std::cout <<"reconx.encodeNx_ " <<  reconx.encodeNx_ << std::endl;
    std::cout <<"reconx.encodeFOV_ " <<  reconx.encodeFOV_ << std::endl;
    std::cout <<"reconx.reconNx_ " <<  reconx.reconNx_ << std::endl;
    std::cout <<"reconx.reconFOV_ " <<  reconx.reconFOV_ << std::endl;
    ...
    if ( h.encoding.size() > 1 ) {
        ...
        std::cout << " omputeTrajectory()  "  << std::endl;
    }
...
int EPIReconXStudyGadget::process(
        ...
        GadgetContainerMessage< hoNDArray< std::complex<float> > >* m2)
{
    ...
    std::ostringstream indice_e1;//ceci est le numero de dynamique ie de 0 à N-1
    indice_e1 << e1;
    str_e1 = indice_e1.str();
    ...
    arma::cx_fmat temp= as_arma_col(*m2->getObjectPtr());
    //std::cout << " impression reconx avant e1 "   <<    e1  << "slice " <<  slice << "  " << size(temp)  << std::endl;
    Gadgetron::SaveMatrixOntheDisk(temp, str_home,  "Tempo/", "reconx_avant_", str_e1, str_s, ".bin");
    ...
    if (hdr_in.encoding_space_ref == 0) {
        //   std::cout << "methode 0  "<<   m1->getObjectPtr()->idx.slice <<  std::endl;
    ...
    *m2->getObjectPtr() = data_out;
    ...
    arma::cx_fmat temp1= as_arma_col(*m2->getObjectPtr());
    //std::cout << " impression reconx apres e1 "   <<    e1  << "slice " <<  slice << "  " << size(temp1)  << std::endl;
    Gadgetron::SaveMatrixOntheDisk(temp1, str_home,  "Tempo/", "reconx_apres_", str_e1, str_s, ".bin");

    // It is enough to put the first one, since they are linked
    if (this->next()->putq(m1) == -1) {
        m1->release();
        GERROR("EPIReconXStudyGadget::process, passing data on to next gadget");
        return -1;
    }
...
GADGET_FACTORY_DECLARE(EPIReconXStudyGadget)
}

...
namespace Gadgetron{
  ...
  EPICorrStudyGadget::EPICorrStudyGadget() {}
  EPICorrStudyGadget::~EPICorrStudyGadget() {}

int EPICorrStudyGadget::process_config(ACE_Message_Block* mb)
{
  
   str_home=Gadgetron::GetHomeDirectory();
  GDEBUG(" $HOME: %s \n", str_home.c_str());
  ...
  GDEBUG_STREAM("EPICorrStudyGadget configured");
  return 0;
}

int EPICorrStudyGadget::process(
      ...
      GadgetContainerMessage< hoNDArray< std::complex<float> > >* m2)
{
  ...
  if (hdr.encoding_space_ref > 0) {
    ...
    if (this->next()->putq(m1) == -1) {
      m1->release();
      GERROR("EPICorrStudyGadget::process, passing data on to next gadget");
      return -1;
    }
    return 0;
  }
  ...
  if (hdr.isFlagSet(ISMRMRD::ISMRMRD_ACQ_IS_PHASECORR_DATA)) {
    ...
    std::ostringstream indice_compteur;
    indice_compteur << compteur_;
    str_compteur = indice_compteur.str();
    
    
    if (compteur_==1  || compteur_==2  || compteur_==3  )
    {
   // std::cout  <<"  navNumber_ "  << navNumber_ <<   "  numNavigators_ : " <<  numNavigators_ <<"  compteur_ "  << compteur_  << " hdr.idx.kspace_encode_step_1 "  << hdr.idx.kspace_encode_step_1  << std::endl;    
   // std::cout  <<"  size "  << size(adata)   << std::endl;    
    }
	...
	indice_slice << hdr.idx.slice;
	str_s = indice_slice.str();
      
    //std::cout  <<"  saving  :  navNumber_ "  << navNumber_ <<   "  numNavigators_ : " <<  numNavigators_ <<"  compteur_ "  << compteur_    << std::endl;
	
    Gadgetron::SaveMatrixOntheDisk(adata, str_home,  "Tempo/", "ligne_", str_compteur,str_s, ".bin");  
    }
    ...
    if (navNumber_ == (numNavigators_-1)) {
      ...
      if (OEPhaseCorrectionMode.value().compare("none")!=0)    // If Odd-Even phase correction is requested
      {
	      ...
	      if (compteur_==3 && hdr.idx.slice==0)
	      {
		//std::cout << " saving ctemp " <<  size(ctemp) << "  real " << real(corrB0_).max() << "  real " << real(corrB0_).min() << std::endl;
		Gadgetron::SaveVectorOntheDisk(ctemp, str_home,  "Tempo/", "ctemp_", str_s, ".bin");
	      }
	      ...
	      GDEBUG_STREAM("tvec = " << tvec(0) << std::endl);
		  ...
		  //std::cout << " phase_poly_coef  "  << phase_poly_coef << std::endl;
		  if (navigatorParameterFilterLength.value() > 1)
		  {
...
void EPICorrStudyGadget::init_arrays_for_nav_parameter_filtering( ISMRMRD::EncodingLimits e_limits )
{
...
float EPICorrStudyGadget::filter_nav_correction_parameter( hoNDArray<float>& nav_corr_param_array,
							    ...
							    bool   filter_in_complex_domain )
{
    ...
    {
	GERROR("cbi_EPICorrStudyGadget::filter_nav_correction_parameter, incorrect number of dimensions of the array.\n");
	return -1;
    }
    ...
    {
	GERROR("cbi_EPICorrStudyGadget::filter_nav_correction_parameter, dimensions of the parameter and weights arrays don't match.\n");
	return -1;
    }
...
void EPICorrStudyGadget::increase_no_repetitions( size_t delta_rep )
{

    GDEBUG_STREAM("cbi_EPICorrStudyGadget WARNING: repetition number larger than what specified in header");
...
GADGET_FACTORY_DECLARE(EPICorrStudyGadget)
}

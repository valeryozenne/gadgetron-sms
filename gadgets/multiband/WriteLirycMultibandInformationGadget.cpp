...
#include "WriteLirycMultibandInformationGadget.h"
...
WriteLirycMultibandInformationGadget::WriteLirycMultibandInformationGadget() {
}

WriteLirycMultibandInformationGadget::~WriteLirycMultibandInformationGadget() {
}

int WriteLirycMultibandInformationGadget::process_config(ACE_Message_Block *mb)
{
    ...
    if (Siemens_Reordering.value())
    {
        // les coupes arrivent par numeros croissants qui correspondent à l'ordre d'acquisition et c'est different de l'ordre géométrique
        // les numeros des coupes multiband sont les n premieres par ex [0 1 2 3 ]
        order_of_acquisition_mb = Gadgetron::map_interleaved_acquisitions(lNumberOfStacks_,  No_Reordering.value() );
    }
    else
    {
        // Minesotta Reordering
        // les coupes arrivent par numeros correspondant à l'ordre d'acquisition et à l'ordre géométrique
        // les numeros des coupes multiband sont les n premiers par ex [ 6 0 7 1 ]
        // parfois la regle est beaucoup plus tordue
        order_of_acquisition_mb = Gadgetron::map_interleaved_acquisitions_mb(lNumberOfStacks_,  No_Reordering.value() );
    }


    // schema de l'acquisition interleaved par ex [1 5 9 3 7 11 0 4 8 2 6 10]
    // TODO changer le nom de cette variable
    order_of_acquisition_sb = Gadgetron::map_interleaved_acquisitions(lNumberOfSlices_,  No_Reordering.value() );
    ...
    guest_order_of_acquisition_mb.set_size(size(order_of_acquisition_sb,0));
    guest_order_of_acquisition_mb.zeros();

    guest_order_of_acquisition_sb.set_size(size(order_of_acquisition_sb,0));
    guest_order_of_acquisition_sb.zeros();
    ...
    if (Siemens_Reordering.value())
    {
        ...
        //dans ce cas l'ordre d'acquisition est croissant

        for (unsigned long a = 0; a < size(order_of_acquisition_sb,0); a++)
        {
            guest_order_of_acquisition_sb(a)=a;
        }


        // les numeros de coupes mb sont les 4 premiers de sb

        for (unsigned long a = 0; a < lNumberOfStacks_; a++)
        {
            slice_number_of_acquisition_mb(a)=guest_order_of_acquisition_sb(a);
        }

    }
    else
    {
        //dans ce cas l'ordre d'acquisition est la table 'indice_sb'

        for (unsigned long a = 0; a < size(order_of_acquisition_sb,0); a++)
        {
            guest_order_of_acquisition_sb(a)=indice_sb(a);
        }
    ...
    if (Siemens_Reordering.value())
    {
        ...
        guest_order_of_acquisition_mb(a)=slice_number_of_acquisition_mb(a);
        }
    ...
    else
    {
        ...
        {
        //std::cout << " a "<< a <<   " order_of_acquisition_mb(a) "<< order_of_acquisition_mb(a) <<   "  acquisition order " <<  slice_number_of_acquisition_mb(order_of_acquisition_mb(a)) << std::endl;
        guest_order_of_acquisition_mb(a)=slice_number_of_acquisition_mb(order_of_acquisition_mb(a));
        }
    ...
    std::cout <<  guest_order_of_acquisition_mb << std::endl;

    real_order_of_acquisition_mb.set_size(size(order_of_acquisition_mb,0));

    real_order_of_acquisition_sb.set_size(size(order_of_acquisition_sb,0));

    mon_fichier = "/home/valery/Dev/Data/fichier.txt";  // je stocke dans la chaîne mon_fichier le nom du fichier à ouvrir

    ofstream fichier(mon_fichier.c_str(), ios::out | ios::trunc);

    if(fichier)  // si l'ouverture a réussi
    {
        std::cout << " ecriture dans mon fichier" << std::endl;
        // instructions
        //fichier << " lNumberOfSlices_ : " << std::to_string(lNumberOfSlices_) << "  lNumberOfStacks_" << std::to_string(lNumberOfStacks_)  << std::endl;

        fichier << " lNumberOfSlices_ : " << std::to_string(lNumberOfSlices_) << "  lNumberOfStacks_" << std::to_string(lNumberOfStacks_)  << "MB_factor_ : " << std::to_string(MB_factor_) << "  Blipped_CAIPI_: " << Blipped_CAIPI_  <<"  MB_Slice_Inc: " << MB_Slice_Inc_  <<  std::endl;

        fichier << " Unknow_2 : " << std::to_string(Unknow_2) << "  Unknow_3 : " << std::to_string(Unknow_3) << "  Unknow_4 : " << std::to_string(Unknow_4) << std::endl;
        fichier << " readout : " << std::to_string(readout) << "  encoding" << encoding  << std::endl;
        fichier << " acceFactorE1_ : " << std::to_string(acceFactorE1_) << "  acceFactorE2_ : " << std::to_string(acceFactorE2_)  << std::endl;
...
int WriteLirycMultibandInformationGadget::process(GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *m1, GadgetContainerMessage<hoNDArray<std::complex<float> > > *m2)
{
    ...
    {
        // std::cout <<" parallel e1 "  <<e1  <<  " slice "  <<slice <<  " rep "<< rep << " phase correction " << std::endl;
    }
    ...
    else
    {
        ...
        if (is_parallel)
        {
            ...
            /// UK
            ///
            if (is_first_scan_in_slice )  {std::cout <<" parallel e1 "  <<e1  <<  " slice "  <<slice <<  " rep "<< rep  << std::endl;}
        ...
        else if (is_acq_single_band )
        {
            ...
            {
                std::cout <<" single_band e1 "  << e1  <<  " slice "  << slice <<  " rep "<< rep<<  " compteur_sb : "<< compteur_sb << std::endl;

                //std::cout << "size"<< size(real_order_of_acquisition_sb)<< std::endl;
        ...
        else if (is_acq_multi_band )
        {
            ...
            if (is_first_scan_in_slice )
            {
                ...
                std::cout<< "compteur_mb" << compteur_mb  << "  "<< lNumberOfStacks_ << std::endl;
                ...
                if (compteur_mb==lNumberOfStacks_)
                {
                    ...
                    {
                        fichier << "     " << a <<  "          " <<  slice_number_of_acquisition_mb(a)  << "             "<< order_of_acquisition_mb(a)<< "                  " <<guest_order_of_acquisition_mb(a)<<  "               " <<real_order_of_acquisition_mb(a)<< "    "  << Is_Equal(guest_order_of_acquisition_mb (a) , real_order_of_acquisition_mb (a))<< std::endl;
                    }
                    ...
                    for (unsigned long a = 0; a < size(order_of_acquisition_sb,0); a++)
                    {
                        fichier << "     " << a <<  "          " <<  order_of_acquisition_sb(a)<< "               " <<guest_order_of_acquisition_sb(a)<<  "               " <<real_order_of_acquisition_sb (a)<<  "    "  << Is_Equal(guest_order_of_acquisition_sb (a) , real_order_of_acquisition_sb (a)) <<std::endl;
                    }
                }
...
std::string  WriteLirycMultibandInformationGadget::Is_Equal(unsigned int a , unsigned int b)
{
    std::string output;
...
void WriteLirycMultibandInformationGadget::get_multiband_parameters(ISMRMRD::IsmrmrdHeader h)
{
...
void WriteLirycMultibandInformationGadget::deal_with_inline_or_offline_situation(ISMRMRD::IsmrmrdHeader h)
{
...
GADGET_FACTORY_DECLARE(WriteLirycMultibandInformationGadget)
}

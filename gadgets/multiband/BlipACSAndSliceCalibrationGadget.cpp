...
#include "BlipACSAndSliceCalibrationGadget.h"
...
BlipACSAndSliceCalibrationGadget::BlipACSAndSliceCalibrationGadget() {
}

BlipACSAndSliceCalibrationGadget::~BlipACSAndSliceCalibrationGadget() {
}

int BlipACSAndSliceCalibrationGadget::process_config(ACE_Message_Block *mb)
{
    ...
    const_phase_1.set_size(1);
    const_phase_1.zeros();

    const_phase_2.set_size(1);
    const_phase_2.zeros();

    arma::fvec fix_value(1);


    fix_value(0)=2*arma::datum::pi/1;
    const_phase_1.set_imag(fix_value);

    fix_value(0)=0;
    const_phase_1.set_real(fix_value);

    fix_value(0)=2*arma::datum::pi/2;
    const_phase_2.set_imag(fix_value);

    fix_value(0)=0;
    const_phase_2.set_real(fix_value);
    ...
    const_shift_1_to_apply_.set_size(1);
    const_shift_1_to_apply_.zeros();

    const_shift_2_to_apply_.set_size(1);
    const_shift_2_to_apply_.zeros();

    const_shift_1_to_apply_=exp(const_phase_1);
    const_shift_2_to_apply_=exp(const_phase_2);

    std::cout << const_shift_1_to_apply_ << std::endl;
    std::cout << const_shift_2_to_apply_ << std::endl;
...
int BlipACSAndSliceCalibrationGadget::process(GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *m1, GadgetContainerMessage<hoNDArray<std::complex<float> > > *m2)
{
    ...
    if (is_parallel)
    {
        ...
        caipi_factor_=2*arma::datum::pi/(2*Blipped_CAIPI_)*shift_;
    ...
    else if (is_acq_single_band)
    {
        ...
        tempo=tempo*const_shift_2_to_apply_*shift_to_apply_.row(e1);
        ...
        else
        {
         ...
         arma::cx_fvec tempo= as_arma_col(*m2->getObjectPtr());
         tempo=tempo*const_shift_1_to_apply_;
...
int BlipACSAndSliceCalibrationGadget::get_blipped_factor(int numero_de_slice)
{
...
void BlipACSAndSliceCalibrationGadget::get_multiband_parameters(ISMRMRD::IsmrmrdHeader h)
{
   ...
   // MB_Slice_Inc_=store_info_special_card(3);
    ...
    std::cout << "acceFactorE1_  " << acceFactorE1_ << " Blipped_CAIPI_ " <<   Blipped_CAIPI_ << std::endl;

    if (acceFactorE1_>1)
    {
    Blipped_CAIPI_=store_info_special_card(4)*acceFactorE1_;
    }
    else
    {
...
void BlipACSAndSliceCalibrationGadget::deal_with_inline_or_offline_situation(ISMRMRD::IsmrmrdHeader h)
{
...
GADGET_FACTORY_DECLARE(BlipACSAndSliceCalibrationGadget)
}

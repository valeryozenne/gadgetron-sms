...
int ExtractThermoIGadget::process_config(ACE_Message_Block* mb) 
{
    ...
    ///------------------------------------------------------------------------
    /// FR définition des drapeaux qui sont transmis par la special card
    /// UK flags definition

    store_info_special_card=Gadgetron::GetInformationFromSpecialCard(h);

    ///------------------------------------------------------------------------
    /// FR permet de distribuer les infos de la special card aux drapeaux en mode offline ou inline
    /// UK

    DealWithInlineOrOfflineSituation();
...
void ExtractThermoIGadget::DealWithInlineOrOfflineSituation(void)
{
    ...
    {
        //on recupere le extract
        doSendBackWhichImages_=store_info_special_card(9);

        if (doSendBackWhichImages_==1)
        {
            doSendBackWhichImages_=25;
            GDEBUG(" Sending back magnitude, phase and temperature images\n " );
        }
        else
        {
            doSendBackWhichImages_=9;
            GDEBUG(" Sending back magnitude and phase images\n " );
        }
    }
}

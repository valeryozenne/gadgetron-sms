...
int GenericReconSplitSMSGadget::process(Gadgetron::GadgetContainerMessage< IsmrmrdReconData >* m1)
{
    ...
    if (is_first_repetition==true)
    {
        ...
        *m2->getObjectPtr() = *m1->getObjectPtr();
        ...
        if (rbit.ref_)
        {
            GDEBUG_STREAM("rbit.ref_  existe  ... ");

            m2->getObjectPtr()->rbit_[e].ref_->clear();
            m2->getObjectPtr()->rbit_[e].ref_ = boost::none;
        }
        else
        {
            GDEBUG_STREAM("rbit.ref_ n'existe pas ... ");
        }
    ...
    else
    {
        ...
        //m1->getObjectPtr()->rbit_[e].data_.headers_=headers_mb; // attention cette ligne est fausse
...
void GenericReconSplitSMSGadget::extract_sb_and_mb_from_data_open(IsmrmrdReconBit &recon_bit, hoNDArray< std::complex<float> >& sb, hoNDArray< std::complex<float> >& mb, hoNDArray< ISMRMRD::AcquisitionHeader > & h_sb, hoNDArray< ISMRMRD::AcquisitionHeader > & h_mb)
{
    ...
    GDEBUG_STREAM("GenericSMSSplitGadget - incoming data array : [RO, E1, E2, CHA, N, S, SLC] - [" << RO << " "<< E1 << " " << E2 << " "<< CHA << " " << N << " " << S << " " << SLC << "]");

    GDEBUG_STREAM("GenericSMSSplitGadget - incoming headers_ array : [E1, E2, N, S, SLC] - [" << hE1 << " " << hE2 << " " << hN << " " << hS << " " << hSLC << "]");

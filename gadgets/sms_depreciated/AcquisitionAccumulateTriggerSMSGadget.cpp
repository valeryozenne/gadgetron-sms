...
#include "AcquisitionAccumulateTriggerSMSGadget.h"
#include "mri_core_data.h"
#include "log.h"

namespace Gadgetron {

    AcquisitionAccumulateTriggerSMSGadget::~AcquisitionAccumulateTriggerSMSGadget()
    {
    ...
    int AcquisitionAccumulateTriggerSMSGadget
        ...
        ::process(GadgetContainerMessage<ISMRMRD::AcquisitionHeader>* m1)
    {
        ...
        if (!(
            ...
            ISMRMRD::FlagBit(ISMRMRD::ISMRMRD_ACQ_USER1).isSet(m1->getObjectPtr()->flags)
            ))
        {
        ...
        if (!(
            ISMRMRD::FlagBit(ISMRMRD::ISMRMRD_ACQ_IS_PARALLEL_CALIBRATION).isSet(m1->getObjectPtr()->flags) ||
            ISMRMRD::FlagBit(ISMRMRD::ISMRMRD_ACQ_IS_PHASECORR_DATA).isSet(m1->getObjectPtr()->flags)||
            ISMRMRD::FlagBit(ISMRMRD::ISMRMRD_ACQ_USER2).isSet(m1->getObjectPtr()->flags)
            ))
        {
            bucket->sb_.push_back(d);
            if (bucket->sbstats_.size() < (espace + 1)) {
                bucket->sbstats_.resize(espace + 1);
            }
            bucket->sbstats_[espace].kspace_encode_step_1.insert(m1->getObjectPtr()->idx.kspace_encode_step_1);
            bucket->sbstats_[espace].kspace_encode_step_2.insert(m1->getObjectPtr()->idx.kspace_encode_step_2);
            bucket->sbstats_[espace].slice.insert(m1->getObjectPtr()->idx.slice);
            bucket->sbstats_[espace].phase.insert(m1->getObjectPtr()->idx.phase);
            bucket->sbstats_[espace].contrast.insert(m1->getObjectPtr()->idx.contrast);
            bucket->sbstats_[espace].set.insert(m1->getObjectPtr()->idx.set);
            bucket->sbstats_[espace].segment.insert(m1->getObjectPtr()->idx.segment);
            bucket->sbstats_[espace].average.insert(m1->getObjectPtr()->idx.average);
            bucket->sbstats_[espace].repetition.insert(m1->getObjectPtr()->idx.repetition);
        }
    ...
    int AcquisitionAccumulateTriggerSMSGadget::process(GadgetContainerMessage<ISMRMRD::ISMRMRD_WaveformHeader>* m1)
    {
    ...
    int AcquisitionAccumulateTriggerSMSGadget::trigger()
    {
    ...
    int AcquisitionAccumulateTriggerSMSGadget::close(unsigned long flags)
    {

        int ret = Gadget::close(flags);

        if (flags != 0) {
            GDEBUG("AcquisitionAccumulateTriggerSMSGadget::close\n");
            trigger();
        }
        return ret;
    }

    GADGET_FACTORY_DECLARE(AcquisitionAccumulateTriggerSMSGadget)
}

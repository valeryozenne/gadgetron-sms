...
if [ $(id -u) -ne 0 ]; then
  ...
  if [ $# -eq 3 ]; then
    ...
    if mountpoint -q ${MOUNT_POINT}; then
        ${MOUNT_POINT}/chroot-root/gadgetron/home/benoit/gadgetron_install_dir/gadgetron4_sms/local/share/gadgetron/chroot/start.sh ${MOUNT_POINT}/chroot-root/gadgetron ${LOG_FILE} &

...
if [ $(id -u) -ne 0 ]; then
  ...
  if [ $# -eq 2 ]; then
    ...
    MAX_MOUNT_TRY=1000
    while [ ${MOUNT_READY} -eq 0 ]; do
      if mountpoint -q ${GLOBAL_MOUNT_POINT} && [ -e ${GLOBAL_MOUNT_POINT}/chroot-root/gadgetron/home/benoit/gadgetron_install_dir/gadgetron4_sms/local/share/gadgetron/chroot/start.sh ]; then
          MOUNT_READY=1
      else
          sleep 0.2
          let MOUNT_TRY++

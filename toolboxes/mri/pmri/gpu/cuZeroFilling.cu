...
namespace Gadgetron{
    ...
    template <class T>
    __global__ void zero_3D_complext (const complext<T> *data_in, complext<T> *data_out, int RO, int E1, int SLC, int scaling, int offset_RO, int offset_E1)
    {
        //RO : size of a column
        //E1 : size of a row
        //SLC : current slice
    ...
    template <class T> void
    execute_zero_3D_complext(complext<T> *data_in, complext<T> *data_out, int RO, int E1, int SLC, int scaling)
    {
        //To modify eventually
        int nbBlocksSlc = 1 << (int)(log2(SLC));
        if (log2(SLC) != (int)(log2(SLC)))
        ...
        zero_3D_complext<<<numBlocks, threadsPerBlock>>>(data_in, data_out, RO, E1, SLC, scaling, offsetRO, offsetE1);
    }
    ...
    template EXPORTGPUPMRI void execute_zero_3D_complext(complext<float> *data_in, complext<float> *data_out, int RO, int E1, int SLC, int scaling);
}
